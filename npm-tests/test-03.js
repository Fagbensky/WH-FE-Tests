...
let dataArray = jsonData.split(', ');

let ageEqualsThirtyTwo = 0;

let file = fs.createWriteStream('./npm-tests/output.txt', {encoding: 'utf8'});

dataArray.forEach(res=>{
    if(res.substring(0, 4) =="age=" && +res.substring(4) == 32)ageEqualsThirtyTwo++;
...
console.log(ageEqualsThirtyTwo);
